...
static void	find(char matriz[MAX_LINE_LENGTH][MAX_LINE_LENGTH], int *total)
{
	for(int x = 0; x < MAX_LINE_LENGTH; x++)
	{
		for(int y = 0; y < MAX_LINE_LENGTH; y++)
		{
			if (matriz[x][y] == 'A')
			{
				if ((matriz[x-1][y-1] == 'M') && (matriz[x-1][y+1] == 'M') 
				&& (matriz[x+1][y-1] == 'S') && (matriz[x+1][y+1] == 'S')) //up
					*total += 1;
				if ((matriz[x+1][y-1] == 'M') && (matriz[x+1][y+1] == 'M') 
				&& (matriz[x-1][y-1] == 'S') && (matriz[x-1][y+1] == 'S')) //down
					*total += 1;
				if ((matriz[x-1][y-1] == 'M') && (matriz[x+1][y-1] == 'M') 
				&& (matriz[x-1][y+1] == 'S') && (matriz[x+1][y+1] == 'S')) //right
					*total += 1;
				if ((matriz[x-1][y+1] == 'M') && (matriz[x+1][y+1] == 'M') 
				&& (matriz[x-1][y-1] == 'S') && (matriz[x+1][y-1] == 'S')) //left
					*total += 1;
			}
		}
	}
